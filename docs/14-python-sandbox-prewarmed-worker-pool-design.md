...
本文原先描述的是后端进程内的 Deno + Pyodide 预热池。当前实现已将这套运行时迁移到独立 `sandbox/` 服务：后端只保留 AST 静态校验和 `execute_python_in_sandbox` 工具入口，通过 `PY_SANDBOX_BASE_URL` 调用 `sandbox` 的 `/execute`；Deno、Pyodide、one-shot worker、预热池和一次性 subprocess fallback 都在 `sandbox` 容器内运行。
...
ready worker 不可用 -> 最多等待 SANDBOX_WORKER_ACQUIRE_TIMEOUT_SECONDS
