...
| AKShare | 开源财经数据接口库，无需 token 获取 A 股行情、财务和参考数据。 | `backend/app/data/ingestors/plugins/akshare_ingestor.py` | 确认数据源的当前条款是否允许目标非商业研究用途、缓存和展示。注意：AKShare 从公开网站采集数据，需遵守各原始数据源的使用条款。默认限流 60 次/分钟，可通过 `AKSHARE_MAX_CALLS_PER_MINUTE` 环境变量调整。 |
...
| AKShare | Open-source financial data interface library that provides A-share market, financial, and reference data without requiring a token. | `backend/app/data/ingestors/plugins/akshare_ingestor.py` | Confirm the current terms permit the intended noncommercial research purpose, caching, and display. Note: AKShare collects data from public websites; operators must comply with the terms of each original data source. Default rate limit is 60 calls/minute, adjustable via `AKSHARE_MAX_CALLS_PER_MINUTE` environment variable. |
