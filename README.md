...
项目使用 PostgreSQL 承载业务数据和市场数据。数据层采用核心表与 JSONB 通用表结合的方式：

- 核心股票数据走结构化表；
- 长尾异构接口走 `raw_akshare` / `raw_tushare` 通用 JSONB 表；
- API 注册表负责描述来源、目标表、更新策略和去重键；
- 定时刷新调度器在应用启动后运行。
...
```text
Best-AI-Trader/
├── backend/              # FastAPI 后端、AI 工作流、交易引擎、数据源、任务与 WebSocket
├── frontend/             # React + Vite + Ant Design 前端
├── docs/                  # 编号项目文档
├── memo/                 # MemoFlux 长期记忆服务
├── docker-compose.yml    # 预构建镜像部署编排
├── docker-compose.dev.yml # 本地源码构建与调试编排
└── nginx.conf            # 前端、API 和 WebSocket 统一入口
```
...
| 模块 | 职责 |
| --- | --- |
| `backend/app/api` | REST API 入口 |
| `backend/app` | 后端核心能力总览，详见 [Backend Capability Map](./backend/app/README.md) |
| `backend/app/ai/llm_engine` | 多 Agent 投研辩论工作流 |
| `backend/app/ai/stock_picker` | AI 智能选股流程 |
| `backend/app/ai/experience` | 决策经验复盘 |
| `backend/app/data` | 数据接入、存储、指标和刷新调度 |
| `backend/app/trading` | 模拟交易服务和纯计算交易引擎 |
| `backend/app/tasks` | 异步任务管理 |
| `backend/app/websocket` | 实时事件推送 |

## 配置说明

后端业务配置集中在 `backend/.env`。字段、默认值和必填说明以 [backend/.env.example](./backend/.env.example) 为准。

主要配置包括：

| 配置 | 作用 |
| --- | --- |
| `PROJECT_NAME` / `PROJECT_VERSION` | 项目显示名和版本 |
| `FIRST_SUPERUSER` / `FIRST_SUPERUSER_EMAIL` / `FIRST_SUPERUSER_PASSWORD` | 初始化管理员账号 |
| `SECRET_KEY` | 后端用于签名和校验 JWT access token 的随机密钥，不是第三方 API Key |
| `litellm/config.yaml` | LiteLLM Proxy 的运行时配置；真实模型和真实 provider key 只写在这里，不提交仓库 |
| `MEMOFLUX_LLM_MODEL` / `MEMOFLUX_LLM_API_KEY` / `MEMOFLUX_LLM_BASE_URL` | MemoFlux LLM 配置；部署时可指向 LiteLLM 的 `memory` 模型别名 |
| `TUSHARE_API` / `TUSHARE_TOKEN` | Tushare 数据源配置，用于行情、财务和基础数据 |
| `DEFAULT_DATA_SOURCE` / `ENABLE_DATA_SOURCE_FAILOVER` | 数据源选择和故障切换配置 |
| `SYSTEM_LANGUAGE` | 系统语言，支持 `zh` / `en` |
| `TAVILY_API_KEY` | Tavily API Key，用于联网搜索、新闻和外部情报补充 |

`backend/.env.example` 中的默认登录账号是 `tradeuser/tradepassword`，只用于初始化和本地试用。
部署到多人或公开环境前必须修改 `FIRST_SUPERUSER_PASSWORD`，并生成足够长的随机 `SECRET_KEY`；
不要使用示例值 `tradepassword` 或 `your-secret-key-here`。

请不要把真实 API Key、token 或密钥提交到仓库。

后端 LLM 接入统一通过 LiteLLM Proxy。仓库只提交 `litellm/config.example.yaml`，真实 `litellm/config.yaml`
由部署环境提供，并通过 Docker secret 注入 LiteLLM 容器。MemoFlux 通过 `memo/.env`
中的 `MEMOFLUX_LLM_*` 连接到 LiteLLM。
LiteLLM 的 OpenAI-compatible API 和管理界面通过 Nginx 暴露在 `4000` 端口，管理界面访问地址为
`http://localhost:4000/ui`。

## 文档导航

- [部署指南](./docs/01-deployment.md)：Docker Compose 服务架构、环境变量、启动停止、验证和排障。
- [Windows WSL2 Docker Engine 部署指南](./docs/04-windows-wsl-docker-engine-deployment.md)：不安装 Docker
...
- 系统仍处于持续开发阶段；以模块 README 和实际代码为准。
- Compose 部署默认只暴露 Nginx `80`；MemoFlux API、PostgreSQL 只在 Compose 内部网络访问，不映射到宿主机。
- 模拟交易引擎不等同于真实券商撮合系统，限价、市价、滑点、停复牌、涨跌停和成交量约束仍需按目标场景继续完善。
- LLM 输出需要经过结构化校验、日志审计和人工风险控制，不能直接视为可执行投资建议。
- 默认部署示例偏向本地研究和模拟用途。如果对公网或多人环境开放，请先按 [安全策略](./SECURITY.md) 收紧鉴权、CORS、插件上传、测试接口和运行时依赖安装。
