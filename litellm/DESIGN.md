...
- 不把真实 provider API key 写入后端 `.env` 或 MemoFlux `.env`；MemoFlux `.env` 只保存 LiteLLM gateway key、
  LiteLLM base URL 和 `openai-compatible` 模型别名。
...
MemoFlux 通过 `memo/.env` 中的 `MEMOFLUX_LLM_*` 感知 LiteLLM endpoint、`openai-compatible` 模型别名和 gateway key。
两者都不感知真实供应商。
...
默认模型别名由 `backend/app/core/config.py` 集中配置，默认使用 `openai-compatible` 和
`openai-compatible-thinking` 两个业务别名。
...
模型别名按能力边界命名，不按供应商命名。后端主应用引用 `openai-compatible`，需要 reasoning 内容的探活和思考场景引用
`openai-compatible-thinking`，MemoFlux 复用 `openai-compatible`；真实模型替换只发生在 LiteLLM 配置中。
...
LLM_MODEL=openai-compatible
LLM_THINKING_MODEL=openai-compatible-thinking
...
- `backend/app/core/config.py` 沿用原有配置名并提供固定默认值，例如 `LLM_PROVIDER`、`LLM_BASE_URL`、
  `LLM_API_KEY`、`LLM_MODEL`、`LLM_THINKING_MODEL`。默认 `LLM_PROVIDER=litellm`、
  `LLM_MODEL=openai-compatible`、`LLM_THINKING_MODEL=openai-compatible-thinking`。
...
LiteLLM 内部 endpoint 和 `openai-compatible` 模型别名：
...
MemoFlux 结构化召回链路继续显式要求可解析 JSON 输出。`openai-compatible` 别名应映射到 JSON 输出稳定的模型。
...
工具调用循环继续消费 LangChain 标准化后的 `AIMessage.tool_calls`。模型别名 `openai-compatible` 必须通过工具调用
...
健康检查、probe、Market Watch 和投研流程默认都使用 `openai-compatible` 别名。若以后需要拆分更多场景，
...
由 LiteLLM 模型别名配置承载；后端只选择 `openai-compatible`、`openai-compatible-thinking` 等别名。

策略如下：

- `openai-compatible` 只映射到已经通过工具调用 smoke test 且 JSON 输出稳定的模型。
- `openai-compatible` 默认配置为 thinking disabled，并设置适合常规 agent、JSON/摘要任务的 `temperature`。
- `openai-compatible-thinking` 配置为 thinking enabled，供需要 reasoning 内容的探活和思考场景使用。
- 若某个供应商能力会破坏 OpenAI-compatible 工具调用 replay，则不能作为 `openai-compatible`。
...
统一 gateway 不能保证所有模型都稳定支持 tool calling。缓解方式是按能力边界定义模型别名，并要求 `openai-compatible`
必须通过工具调用 smoke test。

### Provider-specific 行为差异

不同供应商的扩展字段和工具调用 replay 语义可能不同。缓解方式是后端只支持 OpenAI-compatible 标准字段，
并要求所有 `openai-compatible` 候选模型通过工具调用 smoke test。
...
- MemoFlux 服务通过 `memo/.env` 指向 LiteLLM 内部 endpoint 和 `openai-compatible` 模型别名。
...
- LangChain `bind_tools` 工具调用循环在 `openai-compatible` 上通过 smoke test。
