...
拆分前 Python 沙箱由 `backend/app/ai/agentic/tooling/python_sandbox.py` 调用 Deno 运行
...
- 池耗尽时允许短暂等待，等待失败后返回 `worker_unavailable`，避免自动启动大量一次性子进程打爆内存。
...
采用纯 one-shot 预热池。当前由独立 `sandbox` 服务维护一组已经完成 Pyodide、numpy、pandas 初始化的 Deno worker。沙箱请求到来时，服务从 ready 队列取出一个 worker，将请求通过标准输入发给 worker，worker 执行一次 Python 代码后将结果通过标准输出返回，然后进程退出。服务随后在后台补充新的预热 worker。
...
预热 worker 与一次性 runner 的根本区别是：worker 在执行用户代码之前已经常驻，并且 `sandbox` 服务需要知道 worker 的生命周期状态。不能只把代码写入 stdin 再等待任意 stdout，因为 stdout 同时承担了控制信号、执行结果和错误诊断的传输职责。必须定义明确协议，原因如下。

### 区分 worker 已预热和仍在启动

预热池只能把已经完成 Pyodide 初始化的进程放进 ready 队列。如果没有 ready 协议，服务无法可靠判断 worker 当前处于哪个阶段：
...
因此 worker 启动后必须输出一条结构化 ready 消息。服务只有读到合法 ready 消息，才允许将 worker 放入 ready 队列。
...
这些失败的处理策略不同。例如预热失败应丢弃 worker 并补池；执行超时应 kill worker 并返回 `timed_out=true`；池 acquire 超时应返回 `worker_unavailable`；已经发送用户代码后的协议错误不应再重复执行同一段代码。
...
- acquire ready worker 超时：未发送用户代码，返回 `worker_unavailable`，不自动回退 subprocess。

## 服务执行策略

后端 `execute_python_in_sandbox(code)` 保持现有公开入口并通过 HTTP 调用 `sandbox` 服务；服务内执行顺序：

```text
1. 检查 SANDBOX_ENABLED。
2. validate_python_code(code)。
3. 检查 Deno 可执行文件。
4. 构造请求 JSON。
5. 根据 `execution_mode` 选择 `pooled_worker`、`one_shot_worker` 或 `subprocess`。
6. worker 模式 acquire 成功则走 worker 协议执行。
7. acquire 超时或 worker 池不可用时返回 `worker_unavailable`。
8. 只有显式 `subprocess` 模式才启动一次性 runner。
9. 统一使用 _normalize_response 生成响应。
```

当前同步入口 `execute_python_in_sandbox_sync` 不再保留。生产调用方继续使用后端异步入口 `execute_python_in_sandbox`。
...
SANDBOX_WORKER_ACQUIRE_TIMEOUT_SECONDS: int = 30
SANDBOX_WORKER_STARTUP_TIMEOUT_SECONDS: int = 30
SANDBOX_WORKER_RUNNER_PATH: str = "/app/app/workers/pyodide_one_shot_worker.ts"
```

预热池默认开启并在 `sandbox` 服务启动时分别预热 pooled worker 池和 one-shot 预热池，默认每类启动 1 个 worker。普通 one-shot 预热池和持久 pooled worker 池运行时默认都最多并发启动 2 个 worker，并维持 4 个 ready worker 上限；内存紧张或只做轻量开发调试时，可以关闭 `SANDBOX_PREWARM_ON_STARTUP` 或降低池大小。
...
ready worker 不可用 -> 最多等待 SANDBOX_WORKER_ACQUIRE_TIMEOUT_SECONDS，默认 30 秒
等待后仍不可用 -> 返回 worker_unavailable，不自动启动 subprocess
```

这种策略避免在调用峰值时由 fallback 启动大量 Pyodide 子进程。对于确实需要一次性执行的场景，调用方必须显式传入 `subprocess` 模式。
...
- 执行完成、超时或协议错误后会触发补池。
- acquire 超时且未发送用户代码时返回 `worker_unavailable`，且不启动 subprocess。
- 显式 `subprocess` 模式会启动一次性 Deno runner。
...
- 内存占用增加：Pyodide + pandas worker 常驻内存较高，`SANDBOX_PREWARM_POOL_SIZE` 允许通过环境变量调低或关闭启动预热。
- 补池速度跟不上调用峰值：池空时短等后返回 `worker_unavailable`，由调用方重试或显式选择 `subprocess`。
- 协议实现错误导致响应错配：使用 JSON Lines、`type` 和 `id` 校验，协议错误不重复执行已发送的用户代码。
- 启动时预热拖慢 sandbox 服务：默认启用 startup prewarm，以换取首轮沙箱调用延迟下降；开发环境可关闭 `SANDBOX_PREWARM_ON_STARTUP`。
- worker stdout 被污染：worker 内部捕获用户 stdout/stderr，进程 stdout 只允许输出协议消息。

## 结论

纯 one-shot 预热池是当前 Pyodide + Deno 架构下隔离优先的加速方案。它不减少 Pyodide 初始化的总 CPU 成本，但能将初始化前置到后台，在 ready worker 可用时显著降低用户感知延迟。通过明确的 stdin/stdout JSON Lines 协议，后端可以可靠管理 worker 预热、请求执行、超时和协议错误，同时保持 Deno 最小权限边界。
