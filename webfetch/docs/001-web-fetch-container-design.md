...
新增独立 `webfetch` 容器，对内提供网页渲染、内容选择、HTML/Markdown 转换和 Markdown 正则清理能力。主后端只通过 HTTP 调用该容器，不直接管理浏览器生命周期。
...
迁移目标不是重写或优化 CloakBrowser，而是把现有稳定能力从主后端隔离到 `webfetch` 容器。
...
```env
WEBFETCH_MAX_PAGES=4
WEBFETCH_ENGINE_ACQUIRE_TIMEOUT_MS=30000
WEBFETCH_DEFAULT_TIMEOUT_MS=60000
WEBFETCH_DEFAULT_WAIT_AFTER_MS=5000
```
...
- 超过并发上限时请求最多等待 `WEBFETCH_ENGINE_ACQUIRE_TIMEOUT_MS`。
...
内容质量判断不在 `webfetch` 容器中做，只返回基础元信息。上层可根据 `title`、`content_length`、关键字段和反爬提示自行判断是否升级引擎。

## 9. Docker 与 Compose

`webfetch/Dockerfile` 基于 `python:3.12-slim`。
...
- 如 Camoufox 需要额外系统库，在 `webfetch/Dockerfile` 中单独补齐，不污染主后端镜像。
...
`run.py` 统一封装 Uvicorn 启动参数：固定监听 `0.0.0.0:8010`；开发 Compose 通过 `WEBFETCH_RELOAD=true` 开启 `/app` 热重载，生产 Compose 默认关闭热重载。

`webfetch_runtime_data` 是唯一持久化卷，浏览器安装文件和运行 cache 都归到该卷：
...
Patchright Chromium、Camoufox 浏览器和 CloakBrowser Chromium 在镜像构建阶段下载到 `/runtime`。Docker 首次把空的 `webfetch_runtime_data` 命名卷挂载到 `/runtime` 时，会自动把镜像目录内已有内容复制进卷；后续重建容器会复用同一个卷，避免反复下载浏览器安装文件。

如果 `webfetch_runtime_data` 已经在旧版本中创建且内容为空，Docker 不会再次从新镜像回填；需要删除该卷或手动重新创建后再启动 `webfetch` 服务。
...
WEBFETCH_DEFAULT_ENGINE=cloakbrowser
```

集成方式：

- 保留现有 Agent 工具函数签名。
- 后端 `browser_tool.py` 可逐步改为 HTTP client，调用 `webfetch` 容器 `/fetch`。
...
```bash
docker compose -f docker-compose.dev.yml up -d --build webfetch
...
- 新增 `webfetch` 目录、Dockerfile、FastAPI 服务。
...
- 后端新增 `WEBFETCH_BASE_URL`。
- `browser_tool.py` 优先调用 `webfetch` 容器。
- 旧 CloakBrowser 本地实现保留为短期 fallback。
- 验证市场观察、Agent 工具、PDF/浏览器相关测试。

## 14. 待确认问题

- `webfetch` 容器是否需要 API key 鉴权，即使只在内网访问。
