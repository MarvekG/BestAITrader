...
class FundamentalSource:
    ...
    def _get_financials(self, db: Session, stock_code: str) -> Dict[str, Any]:
        ...
        indicator = db.query(FinancialIndicator).filter(
            ...
            desc(FinancialIndicator.report_date),
            desc(FinancialIndicator.announcement_date),
            desc(FinancialIndicator.updated_at),
        ).first()

        if not indicator or not indicator.data:
            return {}

        # Extract key metrics from JSONB data
        data = indicator.data

        # Use total revenue as a reference for scaling if we encounter absolute values
        total_revenue = data.get("total_revenue") or data.get("operating_income") or 1.0
        ...
        def get_clean_pct(keys: List[str]) -> Optional[float]:
            val = None
            for k in keys:
                val = data.get(k)
                if val is not None:
            ...
            # Heuristic: If value is too large (e.g. > 1000), it's likely an absolute value
            # mapped to a percentage field. Except for some growth rates which could be very high,
            # but margins and ROE > 1000% are rare in A-share abstract data.
            if abs(val) > 1000 and total_revenue > 1.0:
                return (val / total_revenue) * 100

            # If value is already in 0-1 range (e.g. 0.91 for 91%), we might want to scale to 0-100 for consistency
            # but usually these APIs return 91.0 for 91%.
            return val
        ...
        def get_gross_margin() -> Optional[float]:
            ...
            cogs_ratio = get_clean_pct(["cogs_ratio"])
            if cogs_ratio is not None:
                return round(100 - cogs_ratio, 2)

            revenue = get_float(["total_revenue", "operating_income", "revenue"])
            operating_cost = get_float(["operating_cost"])
            if revenue in (None, 0) or operating_cost is None:
                return None
        ...
        payload = {
            ...
            "roe": get_clean_pct(["roe"]),
            "gross_margin": get_gross_margin(),
            "net_margin": get_clean_pct(["net_profit_margin"]),
    ...
    def _get_financial_trend(self, db: Session, stock_code: str) -> Dict[str, Any]:
        """
        分析连续季度财务指标趋势
        Analyze consecutive quarters financial trend
        """
        # 获取最近8个季度的财务数据
        ...
        def extract_metric(ind, keys):
            """从JSONB中提取指标"""
            if not ind.data:
                return None
            for k in keys:
                if ind.data.get(k) is not None:
                    try:
                        return float(ind.data.get(k))
                    except (ValueError, TypeError):
                        pass
            return None

        def extract_clean_pct(ind, keys, *, absurd_limit: Optional[float] = 1000) -> Optional[float]:
            value = extract_metric(ind, keys)
            if value is None:
                return None
        ...
        def extract_gross_margin(ind) -> Optional[float]:
            ...
            cogs_ratio = extract_clean_pct(ind, ["cogs_ratio"])
            if cogs_ratio is not None:
                return 100 - cogs_ratio

            revenue = extract_metric(ind, ["total_revenue", "operating_income", "revenue"])
            operating_cost = extract_metric(ind, ["operating_cost"])
            if revenue in (None, 0) or operating_cost is None:
                return None
            return (revenue - operating_cost) / revenue * 100

        def build_series(metric_name: str, resolver) -> list[Dict[str, Any]]:
            series = []
        ...
        def classify_direction(values: list[float], *, higher_is_better: bool = True, threshold: float = 2.0) -> str:
            if len(values) < 2:
                return "insufficient"
            latest = values[0]
            oldest = values[-1]
            ...
            return "improving" if improved else "weakening"

        def classify_level(direction: str) -> str:
        ...
        def build_trend_module(
            *,
            ...
            primary_points = [item for item in primary_series if item[primary_key] is not None]
            primary_values = [item[primary_key] for item in primary_points]
            direction = classify_direction(
                primary_values,
                higher_is_better=higher_is_better,
                threshold=threshold,
            )
            level = classify_level(direction)
            latest_value = primary_values[0] if primary_values else None
            oldest_value = primary_values[-1] if len(primary_values) >= 2 else None
            change_vs_oldest = (
                round(latest_value - oldest_value, 2)
                if latest_value is not None and oldest_value is not None else None
            )

            if direction == "improving":
                summary = f"{summary_prefix} trend is improving over 8 quarters"
            elif direction == "stable":
                summary = f"{summary_prefix} trend is stable over 8 quarters"
            elif direction == "weakening":
                summary = f"{summary_prefix} trend is weakening over 8 quarters"
            else:
                summary = f"{summary_prefix} trend has insufficient data"

            latest_snapshot = {}
            for metric_key, series in series_map.items():
                latest_point = next((item for item in series if item[metric_key] is not None), None)
                latest_snapshot[metric_key] = latest_point.get(metric_key) if latest_point else None
            ...
            return {
                ...
                "change_vs_oldest": change_vs_oldest,
                "quarters_used": len(primary_values),
                "metrics_available": sum(
                    any(item[metric_key] is not None for item in series_map[metric_key])
                    for metric_key in series_map
                ),
                "series": series_map,
            }

        roe_series = build_series("roe", lambda ind: extract_clean_pct(ind, ["roe"]))
        gross_margin_series = build_series("gross_margin", extract_gross_margin)
        net_margin_series = build_series("net_margin", lambda ind: extract_clean_pct(ind, ["net_profit_margin"]))
        revenue_yoy_series = build_series(
            "total_revenue_yoy",
            lambda ind: extract_clean_pct(ind, ["total_revenue_yoy"], absurd_limit=None),
        )
        profit_yoy_series = build_series(
            "net_profit_yoy",
            lambda ind: extract_clean_pct(ind, ["net_profit_yoy"], absurd_limit=None),
        )
        deducted_profit_yoy_series = build_series(
            "net_profit_dedt_yoy",
            lambda ind: extract_clean_pct(ind, ["net_profit_dedt_yoy"], absurd_limit=None),
        )
        debt_ratio_series = build_series("debt_to_asset", lambda ind: extract_clean_pct(ind, ["debt_to_assets_ratio"]))

        recent_quarters = []
        for idx in range(len(indicators)):
            recent_quarters.append(
                self._drop_empty_context_values({
                    "date": roe_series[idx]["date"],
                    "roe": roe_series[idx]["roe"],
                    "gross_margin": gross_margin_series[idx]["gross_margin"],
                    "net_margin": net_margin_series[idx]["net_margin"],
                    "total_revenue_yoy": revenue_yoy_series[idx]["total_revenue_yoy"],
                    "net_profit_yoy": profit_yoy_series[idx]["net_profit_yoy"],
                    "net_profit_dedt_yoy": deducted_profit_yoy_series[idx]["net_profit_dedt_yoy"],
                    "debt_to_asset": debt_ratio_series[idx]["debt_to_asset"],
                })
            )

        profitability_trend = build_trend_module(
            summary_prefix="Profitability",
            primary_key="roe",
        ...
        growth_trend = build_trend_module(
            summary_prefix="Growth",
            primary_key="net_profit_yoy",
            higher_is_better=True,
            threshold=5.0,
            series_map={
                "total_revenue_yoy": revenue_yoy_series,
                "net_profit_yoy": profit_yoy_series,
                "net_profit_dedt_yoy": deducted_profit_yoy_series,
            },
        )
        leverage_trend = build_trend_module(
            summary_prefix="Leverage",
            primary_key="debt_to_asset",
            higher_is_better=False,
            threshold=2.0,
            series_map={
                "debt_to_asset": debt_ratio_series,
            },
        )

        levels = [
            profitability_trend["level"],
            growth_trend["level"],
            leverage_trend["level"],
        ]
        if levels.count("Strong") >= 2:
            overview_level = "Strong"
        elif "Watchlist" in levels:
            overview_level = "Watchlist"
        elif levels.count("Stable") >= 2:
            overview_level = "Stable"
        else:
        ...
        result = {
            "overview": {
                ...
                "recent_quarters_used": len(recent_quarters),
            },
