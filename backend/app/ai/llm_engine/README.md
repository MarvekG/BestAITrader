...
`llm_engine` 是单股 AI 投研决策引擎。它把一次股票分析拆成“事实上下文构建、第一层专业分析、第二层战略辩论、PM 最终决策、审计持久化”几个阶段，避免模型直接给出不可追踪的买卖结论。
...
flowchart TD
    ...
    ContextLayers --> Adapters["context/adapters.py<br/>Agent inputs"]

    Adapters --> Layer1["Layer 1<br/>news / policy / sentiment / vertical analysts"]
    Layer1 --> Layer2["Layer 2<br/>bull-bear debate<br/>aggressive / conservative / neutral"]
    Layer2 --> PM["Portfolio Manager<br/>PMDecision + optional trade tool"]
...
| [`context/adapters.py`](./context/adapters.py) | 将统一 context 转成各 Agent 的最小输入。 |
| [`agents/base.py`](./agents/base.py) | Agent 通用执行器，封装 LLM、工具循环、摘要、校验和重试。 |
...
flowchart TD
    ...
    Round21 -->|errors| End
    Round21 -->|ok| PMNode["portfolio_management<br/>PortfolioManagerAgent"]
    PMNode --> End
```

说明：

- `fetch_context` 成功后，第一层分析并行分叉。
...
Agent 不直接依赖数据库表结构，也不依赖旧式 context slice。所有 Agent 输入都从 `context/adapters.py` 派生。
...
当前图中只有 `strategic_round_2_1`。`strategic_round_2_rebuttal(...)` 只是兼容历史调用，不再作为图节点写入新 stage。
...
2. 拼接角色 system prompt、skills catalog 和 context JSON。
3. 绑定工具进入工具调用循环。
...
2. 确保缺失数据用 `status` 表达。
3. 在 `context/adapters.py` 中只把必要字段暴露给对应 Agent。
...
4. [`context/adapters.py`](./context/adapters.py)
5. [`agents/base.py`](./agents/base.py)
6. [`agents/specialists.py`](./agents/specialists.py)
7. [`agents/strategic.py`](./agents/strategic.py)
8. [`agents/governance.py`](./agents/governance.py)
9. [`models.py`](./models.py)
10. [`prompts/templates.py`](./prompts/templates.py)
