...
def _resolve_session_user_id(session_id: Optional[str]) -> int | None:
    ...
    with SessionLocal() as db:
        session_obj = db.query(SessionModel).filter(
            SessionModel.session_id == UUID(session_id)
        ).first()
        if session_obj is None:
            return None
        return int(session_obj.user_id)


def _update_session_status(db, session_id: str, status: str):
    """更新会话状态。
...
async def run_analysis_task(
    ...
    try:
        user_token = set_current_user_id(_resolve_session_user_id(session_id))
        ...
        with SessionLocal() as db:
            if final_state.get("errors"):
                ...
                if session_id:
                    _update_session_status(db, session_id, "completed")
