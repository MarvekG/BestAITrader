...
| 目标价与字段语义 | 说明估值方法、核心假设、上行/下行空间和失效条件；`stop_loss` 是系统写入持仓并由盘中扫描监控的最近风险复议线，触发后只启动复议，不等于自动清仓；更远的硬失效价、估值失效价或未来重新入场参考价只能写在正文中，不得与结构化 `stop_loss` 混用；卖出或清仓时 `stop_loss` 记录本轮退出的触发边界；`take_profit` 必须大于 0，买入或正仓持有时需形成正向收益闭环，卖出或清仓时作为原目标价或放弃的估值参考；若 `decision="hold"` 且正文计划在较近价格部分止盈或触发复议，`take_profit` 必须取最近一个需要系统监控的价格，而不是远期乐观目标；`holding_horizon_days` 必须为正整数，卖出或清仓时表示退出后复核窗口。 |
...
| 事实仲裁处理 | 若有 `fact_arbitration_report`，说明采用口径、未解决事实及影响。未解决事实逐项标明降权、补证后再行动或转化为**条件性触发器**（只能写为"若未来核实/发生则…触发某动作"，不得当作当前已成立的利空或利好）。**未解决事实不得计入置信度加分项**，也不得与已裁决事实并列为"N 重利空/利好共振"等强证据叙事；未解决事实只能作为扣分项、观察项或条件性触发器。不得把未解决事实作为强买/强卖依据。 |
