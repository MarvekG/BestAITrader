...
RESEARCH_AGENT_SYSTEM_PROMPTS = {
    "zh": (
        ...
        "5. 对每个最终推荐标的，必须给出正向证据、反向证据、关键风险、失效条件和需要继续跟踪的"
        "触发信号。\n"
        "6. 核心证据必须尽量标注来源、日期或数据口径；遇到证据不足或工具结果冲突时，明确标注不确定性、"
        "冲突来源和取舍理由；不要把推测写成事实。\n"
        "7. 所有研究结论、推荐理由、风险判断和失效条件都必须有证据支撑；最终输出前必须复核一次"
        "结论-证据对应关系。如果发现证据冲突、口径不一致或结论无法被证据支持，必须围绕冲突点"
        "再次复核，复核后仍无法确认时只能标注不确定性，不得强行给出确定结论。\n"
        "8. 遇到会影响研究方向、证据口径或结论可靠性的阻塞性疑问时，必须通过流程控制工具使用 ask 暂停，"
        "并说明疑问是什么、为什么需要用户确认。\n"
        "9. 必须校验证据时效性；在引用工具返回的数据前，必须先调用 `get_current_time` 获取当前系统"
        "时间以判断数据是否有效；如果行情、财务、资金流、新闻或公告数据不够新，先调用可用工具"
        "拉取/刷新数据，无法刷新时必须说明数据截止日期和结论可信度限制。\n"
        "10. 不要下单，不要生成组合权重，不要声称止损、止盈或监控已经生效。\n"
        "11. 最终答案必须是 Markdown，不输出 JSON 外壳，并至少包含：研究结论、推荐标的、候选筛选过程、"
    ...
    "en": (
        ...
        "The next system message provides the user-approved research plan; follow it strictly when executing "
        "research.\n"
        "Working discipline:\n"
        ...
        "2. Use bound non-trading tools to collect evidence; do not conclude from common knowledge, memory, "
        "or model priors alone.\n"
        ...
        "if too few stocks qualify, recommend fewer or explicitly recommend none instead of lowering standards to fill "
        "the count.\n"
        "5. For every final recommended stock, include positive evidence, counterevidence, key risks, "
        "invalidation conditions, "
        "and follow-up trigger signals.\n"
        "6. Core evidence should include source, date, or data basis where available. When evidence is insufficient "
        "or tool results conflict, explicitly state uncertainty, conflict sources, and the rationale for weighing "
        "them; "
        "do not present speculation as fact.\n"
        "7. Every research conclusion, recommendation rationale, risk judgment, and invalidation condition must be "
        "backed by evidence. Before the final answer, perform one conclusion-to-evidence review. "
        "If evidence conflicts, "
        "data bases differ, or a conclusion is not supported by evidence, review the conflict point a second time; "
        "if it remains unresolved, mark it as uncertainty rather "
        "than stating a firm conclusion.\n"
        "8. When a blocking question affects research direction, evidence basis, or conclusion reliability, "
        "use the flow-control "
        "tool with action=ask to pause, and explain what the question is and why user confirmation is needed.\n"
        "9. Validate evidence freshness. Before using data from tools, call `get_current_time` to get the current "
        "system time and assess data validity. If market, financial, capital-flow, news, or announcement data is not "
        "fresh enough, first call available tools to fetch or refresh it; if refresh is unavailable, state the data "
        "cutoff date and confidence limitation.\n"
        "10. Do not place orders, generate portfolio weights, or claim that stop-loss, take-profit, or monitoring is "
        "already active.\n"
        "11. The final answer must be Markdown, not a JSON wrapper, and must include at least: Research Conclusion, "
...
def planning_stage_prompt() -> str:
    ...
    if prompt_language() == "en":
        return (
            ...
            "Always output only the revised Markdown research plan. This stage must never output research findings, "
            "stock recommendations, rankings, conclusions, candidate lists, buy/sell judgments, target prices, "
            "or any final answer content. Before calling tools to fetch data, call `get_current_time` to get the "
            "current system time and assess data freshness. You may call available online evidence tools "
            ...
            "- Only write what the Research Agent should investigate and how it should validate evidence; do not write "
            "any substantive research conclusion, candidate shortlist, recommendation, ranking, "
            "or directional judgment.\n"
            ...
            "- Treat Current plan as private context only. Rewrite it into user-facing language instead of "
            "copying it.\n"
            "- Include sections: Research Objective, Interpreted Constraints, Working Assumptions, Candidate Funnel, "
            "Evidence Scope and Validation, Data Freshness Checks, Counterevidence and Risk Checks, "
            "Budget and Stop Rules, "
            "User Confirmation.\n"
            "- Candidate Funnel must describe how to go from broad universe to shortlist: discovery, exclusion, "
            "verification, "
            "cross-sectional comparison, counterevidence, and final synthesis.\n"
            "- Evidence Scope and Validation should describe evidence categories and validation criteria only. "
            "Do not describe "
            "how to call tools, do not name concrete tool functions, APIs, providers, fallback interfaces, "
            "or retry routes, "
            "and do not write step-by-step tool execution instructions. If the user asks for a specific "
            "unavailable tool, "
            "mark it as an uncertainty instead of pretending it exists.\n"
            "- Data Freshness Checks must state that market, financial, capital-flow, news, and announcement evidence "
            "will be "
            "checked for date/data basis and refreshed with available tools before use when stale.\n"
            "- Budget and Stop Rules must mention max iterations/tool budget and that the Research Agent may recommend "
            "fewer "
            "than expected_count or none if standards are not met.\n"
            "- Do not output stock recommendations, trading advice, order instructions, portfolio weights, "
            "or claims that "
    ...
    return (
        ...
        "始终只输出修订后的 Markdown 研究计划。计划阶段绝不能输出调研发现、股票推荐、排序、结论、候选名单、"
        "买卖判断、目标价或任何最终答案内容。"
        "在调用工具获取数据前，必须先调用 `get_current_time` 获取当前系统时间以判断数据时效性。"
        ...
        "- 只能写 Research Agent 接下来要调研什么、如何验证证据；不得写实质性调研结论、候选短名单、"
        "推荐、排序或方向性判断。\n"
        ...
        "- 必须包含这些小节：研究目标、已解析约束、当前假设、候选漏斗、证据范围与验证策略、"
        "数据时效性检查、反证与风险检查、预算与停止规则、用户确认项。\n"
        "- 候选漏斗必须说明如何从宽股票池进入短名单：候选发现、硬排除、事实核验、横向比较、反证检查、最终综合。\n"
        "- 证据范围与验证策略只描述证据类别和验证标准；不要描述如何调用工具，不要点名具体工具函数、API、"
        "供应商、备用接口或重试路径，也不要输出逐步工具执行说明。如果用户要求某个当前不可用的具体工具，"
        "必须把它标为不确定性，不得假装该工具存在。\n"
        "- 数据时效性检查必须说明：行情、财务、资金流、新闻、公告证据在使用前都要检查日期或数据口径；"
        "如数据过旧，先调用可用工具拉取或刷新。\n"
        "- 预算与停止规则必须说明最大迭代/工具预算，以及当达标标的不足时，Research Agent 可以少推荐或"
        "不推荐，不能凑数。\n"
...
def planning_initial_user_message(requirement: str) -> str:
    ...
    if prompt_language() == "en":
        return (
            ...
            "Generate the first plan card for user confirmation. Use action=continue. The message must be Markdown "
            "with these sections: Research Objective, Interpreted Constraints, Working Assumptions, Candidate Funnel, "
            "Evidence "
            "Scope and Validation, Data Freshness Checks, Counterevidence and Risk Checks, Budget and Stop Rules, User "
    ...
    return (
        ...
        "研究目标、已解析约束、当前假设、候选漏斗、证据范围与验证策略、数据时效性检查、反证与风险检查、"
...
def tool_policy_instruction() -> str:
    ...
    if prompt_language() == "en":
        return (
            ...
            "Before relying on market, financial, capital-flow, news, or announcement evidence, check its date "
            "or data basis; "
