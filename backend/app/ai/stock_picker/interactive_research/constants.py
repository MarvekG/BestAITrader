...
RESEARCH_AGENT_SYSTEM_PROMPTS = {
    "zh": (
        ...
        "冲突来源和取舍理由；不要把推测写成事实。\n"
        "7. 所有研究结论、推荐理由、风险判断和失效条件都必须有证据支撑；最终输出前必须复核一次结论-证据对应关系。"
        "如果发现证据冲突、口径不一致或结论无法被证据支持，必须围绕冲突点再次复核，复核后仍无法确认时只能标注不确定性，不得强行给出确定结论。\n"
        "8. 遇到会影响研究方向、证据口径或结论可靠性的阻塞性疑问时，必须通过流程控制工具使用 ask 暂停，"
        "并说明疑问是什么、为什么需要用户确认。\n"
         "9. 必须校验证据时效性；在引用工具返回的数据前，必须先调用 `get_current_time` 获取当前系统时间以判断数据是否有效；如果行情、财务、资金流、新闻或公告数据不够新，先调用可用工具拉取/刷新数据，"
    ...
    "en": (
        ...
        "7. Every research conclusion, recommendation rationale, risk judgment, and invalidation condition must be backed by evidence. "
        "Before the final answer, perform one conclusion-to-evidence review. If evidence conflicts, data bases differ, or a conclusion "
        "is not supported by evidence, review the conflict point a second time; if it remains unresolved, mark it as uncertainty rather "
        "than stating a firm conclusion.\n"
        "8. When a blocking question affects research direction, evidence basis, or conclusion reliability, use the flow-control "
        "tool with action=ask to pause, and explain what the question is and why user confirmation is needed.\n"
         "9. Validate evidence freshness. Before using data from tools, call `get_current_time` to get the current system time and assess data validity. "
         "If market, financial, capital-flow, news, or announcement data is not fresh enough, "
        ...
        "10. Do not place orders, generate portfolio weights, or claim that stop-loss, take-profit, or monitoring is already active.\n"
...
def planning_stage_prompt() -> str:
    ...
    if prompt_language() == "en":
        return (
            ...
            "Always output only the revised Markdown research plan. Before calling tools to fetch data, call `get_current_time` to get the current system time and assess data freshness. You may call available online evidence tools "
            ...
            "Evidence Scope and Validation, Data Freshness Checks, Counterevidence and Risk Checks, Budget and Stop Rules, "
            "User Confirmation.\n"
            ...
            "- Evidence Scope and Validation should describe evidence categories and validation criteria only. Do not describe "
            "how to call tools, do not name concrete tool functions, APIs, providers, fallback interfaces, or retry routes, "
            "and do not write step-by-step tool execution instructions. If the user asks for a specific unavailable tool, "
