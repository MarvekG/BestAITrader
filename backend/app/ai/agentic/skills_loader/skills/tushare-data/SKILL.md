...
## 天枢智投（Best-AI-Trader）调用方式

本仓库的数据调用脚本入口是：`scripts/call_tushare.py`。

运行前需要在后端配置中提供可用的 Tushare Token，相关 Python 依赖由后端依赖文件统一管理，不在 skill manifest 中声明。

调用 `scripts/call_tushare.py` 前必须先获取最新接口文档：

1. 调用 `load_skill` 读取本 `SKILL.md`。
2. 调用 `read_skill_file` 读取 `references/数据接口.md`，确认候选接口。
3. 从 `references/数据接口.md` 中找到候选接口的在线文档 URL。
4. 必须调用 `scripts/fetch_sdk_docs.py` 获取该接口的最新在线文档。
5. 只有在阅读最新在线文档并确认 Tushare `interface_name`、必填参数、可选参数和返回字段后，才能调用 `scripts/call_tushare.py`。

不要仅凭本地 reference、记忆或接口名直接调用 `scripts/call_tushare.py`。
...
```bash
python scripts/call_tushare.py \
  --interface-name daily \
  --params-json '{"ts_code":"000001.SZ","start_date":"20240101","end_date":"20240131"}'
```

或者把同一个 JSON 对象作为第一个命令行参数传入。

`interface_name` 是 Tushare 接口名。`params` 是传给该接口的 kwargs，由 LLM 根据本 skill 文档和用户请求生成。

如果本地 reference 不足，需要临时查看 SDK/接口网页文档，可以调用 `scripts/fetch_sdk_docs.py`。它支持
stdin JSON、argv JSON 和命令行参数，输出抓取到的正文，也可以写入 skill 内部文件：

```json
{
  "url": "https://tushare.pro/document/2",
  "format": "text",
  "output_path": "references/sdk-docs.md"
}
```
...
```bash
python scripts/fetch_sdk_docs.py --url https://tushare.pro/document/2 --output-path references/sdk-docs.md
```
