...
class PrewarmedSandboxPool:
    ...
    async def prewarm(self) -> None:
        """启动后台 worker 补池任务。"""
        if shutil.which(settings.PY_SANDBOX_DENO_EXECUTABLE) is None:
            logger.warning(
                "prewarmed sandbox pool skipped because Deno executable is unavailable",
                extra={"deno_executable": settings.PY_SANDBOX_DENO_EXECUTABLE},
            )
            return
    ...
    async def _acquire(self) -> PrewarmedSandboxWorker:
        ...
        try:
            return await asyncio.wait_for(
                self._ready.get(),
                timeout=settings.PY_SANDBOX_WORKER_ACQUIRE_TIMEOUT_SECONDS,
            )
    ...
    async def _replenish(self) -> None:
        ...
        async with self._lock:
            target = max(1, int(settings.PY_SANDBOX_PREWARM_POOL_SIZE or 1))
            max_starting = max(1, int(settings.PY_SANDBOX_PREWARM_MAX_STARTING or 1))
    ...
    async def _read_ready_message(self, process: asyncio.subprocess.Process) -> Dict[str, Any]:
        ...
        deadline = time.monotonic() + settings.PY_SANDBOX_WORKER_STARTUP_TIMEOUT_SECONDS
        while True:
...
def _build_worker_command() -> list[str]:
    ...
    """
    runner_path = Path(settings.PY_SANDBOX_WORKER_RUNNER_PATH)
