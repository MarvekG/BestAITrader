...
def _unit_keys_to_list(unit_key: Any) -> list[str]:
    """把单位配置统一转换为 i18n key 列表。
    ...
    if isinstance(unit_key, str):
        return [unit_key]
    if isinstance(unit_key, list):
        return [item for item in unit_key if isinstance(item, str)]
    return []


def _source_unit_key(unit_key: Any, scale: float) -> Any:
    """根据展示倍率推断数据库原始单位 key。

    Args:
        unit_key: 展示单位 i18n key。
        scale: 原始值转换为展示值的倍率。

    Returns:
        数据库原始单位 i18n key；无法推断时返回展示单位 key。
    """
    keys = _unit_keys_to_list(unit_key)
    if "units.cny" in keys and scale in {1e-8, 1e-9, 0.00000001, 0.000000001}:
        return "units.cny"
    if "units.shares" in keys and scale in {1e-8, 1e-9, 0.00000001, 0.000000001}:
        return "units.shares"
    return unit_key
