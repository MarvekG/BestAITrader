...
from app.api.ownership import get_owned_position, get_owned_session
...
async def get_positions(
    ...
    try:
        ...
        # 获取最新价格的子查询
        latest_market_subquery = db.query(
...
async def get_position(
    ...
    """Get single position"""
    try:
        position = get_owned_position(db, position_id, current_user)
        ...
        position_info = {
            ...
            "session_id": str(position.session_id),
            "stock_code": position.stock_code,
            "stock_name": db.query(StockBasic.name).filter(StockBasic.stock_code == position.stock_code).scalar() or "Unknown",
            "current_shares": position.total_shares,
            "available_shares": available_shares,
            "frozen_shares": frozen_shares,
            "avg_cost": position.avg_cost,
