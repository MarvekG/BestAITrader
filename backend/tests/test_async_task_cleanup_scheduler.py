...
@pytest.mark.asyncio
async def test_async_task_cleanup_deletes_tasks_older_than_configured_retention(
    async_db_session,
    monkeypatch,
) -> None:
    now = datetime(2026, 5, 27, 12, 0)
    _add_task(async_db_session, task_id="old", created_at=now - timedelta(days=31))
    _add_task(async_db_session, task_id="recent", created_at=now - timedelta(days=29))
    ...
    result = await scheduler_module.cleanup_old_async_tasks()

    remaining_task_ids = (
        await async_db_session.execute(select(AsyncTask.task_id).order_by(AsyncTask.task_id))
    ).scalars().all()
...
def test_scheduled_task_registry_includes_async_task_cleanup_job() -> None:
