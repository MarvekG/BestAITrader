...
def test_performance_summary_returns_latest_snapshot(client, db_session, auth_headers) -> None:
    """绩效摘要接口应返回当前用户最新账户快照。"""
...
def test_performance_equity_curve_returns_snapshots(client, db_session, auth_headers) -> None:
    """净值曲线接口应按日期返回账户和基准曲线。"""
    user = db_session.query(User).filter(User.username.like("test_%")).first()
    account = Account(
        user_id=user.id,
