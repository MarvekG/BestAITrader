...
async def _get_test_user(db) -> User:
    """读取当前测试登录用户。"""
    return (await db.execute(select(User).where(User.username.like("test_%")))).scalars().first()


def test_portfolio_overview_returns_weights_and_industries(client, test_db, run_async, auth_headers) -> None:
    """组合概览接口应返回动态估值、单股权重和行业分布。"""
    async def _seed_data() -> None:
        async with test_db() as db:
            user = await _get_test_user(db)
            account = Account(
                user_id=user.id,
...
def test_portfolio_overview_returns_weighted_volatility_metrics(client, test_db, run_async, auth_headers) -> None:
