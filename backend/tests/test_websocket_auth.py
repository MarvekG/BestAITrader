...
def test_global_websocket_accepts_one_time_ticket(client, auth_headers):
    ticket_response = client.post("/ws-ticket/session-1", headers=auth_headers)
    assert ticket_response.status_code == 200
    ticket = ticket_response.json()["ticket"]

    with client.websocket_connect(f"/ws/session-1?ticket={ticket}") as websocket:
        websocket.send_json(
            {
                "type": "ping",
    ...
    with pytest.raises(WebSocketDisconnect) as exc_info:
        with client.websocket_connect(f"/ws/session-1?ticket={ticket}"):
            pass
