...
async def test_search_tries_next_key_when_response_is_not_200(monkeypatch):
    monkeypatch.setattr("app.ai.agentic.tooling.news_plugins.tavily.get_data_source_config_list", AsyncMock(return_value=["bad-key", "good-key"]))
    used_keys = []
...
async def test_search_returns_fatal_error_when_all_keys_fail(monkeypatch):
    monkeypatch.setattr("app.ai.agentic.tooling.news_plugins.tavily.get_data_source_config_list", AsyncMock(return_value=["expired-key"]))
