...
@pytest.mark.asyncio
async def test_create_first_snapshot_uses_zero_returns(async_db_session) -> None:
    ...
    async with database_module.AsyncSessionLocal() as async_db:
        ...
        snapshot = await create_account_equity_snapshot(
            async_db,
...
@pytest.mark.asyncio
async def test_create_followup_snapshot_calculates_returns_and_drawdown(async_db_session) -> None:
    ...
    async with database_module.AsyncSessionLocal() as async_db:
        async_account = await async_db.get(Account, account.account_id)
        await create_account_equity_snapshot(async_db, account=async_account, snapshot_date=date(2026, 5, 21))
    ...
    async with database_module.AsyncSessionLocal() as async_db:
        async_account = await async_db.get(Account, account.account_id)
        snapshot = await create_account_equity_snapshot(async_db, account=async_account, snapshot_date=date(2026, 5, 22))
...
@pytest.mark.asyncio
async def test_recreating_same_day_snapshot_updates_without_duplicate_peak(async_db_session) -> None:
    ...
    assert snapshot.max_drawdown == Decimal("-0.10000000")
    count = await async_db_session.scalar(select(func.count(IndexDaily.index_code)))
    assert count == 2
