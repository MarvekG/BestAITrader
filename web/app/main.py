...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    settings = get_settings()
    logger.info(
        "web fetcher service started",
        extra={
            "max_pages": settings.WEB_MAX_PAGES,
            "engine_acquire_timeout_ms": settings.WEB_ENGINE_ACQUIRE_TIMEOUT_MS,
            "default_timeout_ms": settings.WEB_DEFAULT_TIMEOUT_MS,
            "default_wait_after_ms": settings.WEB_DEFAULT_WAIT_AFTER_MS,
            "default_download_timeout_seconds": settings.WEB_DEFAULT_DOWNLOAD_TIMEOUT_SECONDS,
        },
    )
    try:
        yield
    finally:
        logger.info("web fetcher service stopping", extra={"engine_count": len(engine_registry._engines)})
        await engine_registry.close()
        logger.info("web fetcher service stopped")
